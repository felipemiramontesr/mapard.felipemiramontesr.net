    ...
    if (isLoading && feed.length === 0) {
        return (
            <div className="w-full flex justify-center py-8">
                <div className="w-6 h-6 border-2 border-ops-cyan border-t-transparent rounded-full animate-spin"></div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full flex flex-col gap-4 mt-6">
            <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
                <h3 className="text-[11px] md:text-sm font-bold text-white tracking-widest uppercase flex items-center gap-2">
                    <Globe className="text-ops-cyan w-4 h-4 flex-shrink-0" />
                    <span>Buzón Táctico Global</span>
                </h3>
                <span className="text-[10px] text-ops-text_dim font-mono bg-white/5 px-2 py-1 rounded">
                    {feed.length} ALERTAS
                </span>
            </div>
