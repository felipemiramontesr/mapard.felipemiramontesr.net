...
const StatusTerminal: React.FC<StatusTerminalProps> = ({ logs, isVisible, onReset }) => {
    ...
    return (
        <motion.div
            ...
            className="ops-card mt-8 font-mono text-xs overflow-hidden border-ops-radioactive/30 shadow-[0_0_20px_rgba(57,255,20,0.1)]"
        >
            <div className="bg-ops-bg_alt/90 px-4 py-2 flex items-center justify-between border-b border-white/5 mb-2">
                <div className="flex items-center gap-2 text-ops-radioactive animate-pulse">
                ...
                <div className="flex gap-1.5">
                    ...
                    <div className="w-2 h-2 rounded-none bg-ops-radioactive" />
                </div>
            </div>
                    ...
                    {logs.map((log) => (
                        <motion.div
                            ...
                            className={`flex gap-3 ${log.type === 'error' ? 'text-ops-danger font-bold' :
                                log.type === 'success' ? 'text-ops-radioactive font-bold' :
            ...
            {isCompleted && onReset && (
                ...
                >
                    <button
                        onClick={onReset}
                        className="w-full py-3 bg-ops-radioactive/10 border border-ops-radioactive text-ops-radioactive font-bold tracking-[0.2em] hover:bg-ops-radioactive/20 transition-all uppercase text-sm animate-pulse"
                    >
