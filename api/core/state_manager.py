...
class StateManager:
    ...
    def _init_empty_state(self) -> None:
        ...
        self.data = {"clients": {}, "intakes": {}, "reports": {}, "logs": []}
        self.save_data()
    ...
    def create_client(
        self, client_id: str, name_full: str, client_type: str = "PF"
    ) -> str:
    ...
    def _get_or_create_client_id(self, client_name: str) -> str:
        ...
        while new_id in self.data["clients"]:
            new_id = (
                hashlib.sha256((client_name + new_id).encode("utf-8"))
                .hexdigest()[:7]
                .upper()
            )
    ...
    def create_intake(
        ...
        self.add_event_log(
            "INTAKE", intake_id, "CREATE", None, "GENERADO", actor=requested_by
        )
        self.save_data()
        return intake_id

    def update_intake(
        self, intake_id: str, to_status: str, actor: str = "SYSTEM"
    ) -> None:
        ...
        self._validate_transition(
            from_status,
            to_status,
            self.INTAKE_STATUSES,
            {"GENERADO": ["AUTORIZADO"], "AUTORIZADO": ["EJECUTADO"]},
        )
        ...
        self.add_event_log(
            "INTAKE", intake_id, "STATUS_CHANGE", from_status, to_status, actor=actor
        )
        self.save_data()
    ...
    def update_report_status(
        ...
        self.add_event_log(
            "REPORT", report_id, "STATUS_CHANGE", from_status, to_status, actor=actor
        )
        self.save_data()
    ...
    def _validate_transition(
        self, f: str, t: str, allowed_statuses: List[str], rules: Dict[str, List[str]]
    ) -> None:
    ...
    def list_authorized_intakes_by_priority(self) -> List[Dict[str, Any]]:
        ...
        authorized = [
            i
            for i in self.data["intakes"].values()
            if i["intake_status"] == "AUTORIZADO"
        ]
