...
class QCModule:
    ...
    def validate_report(self, base_name: str, client_name: str, client_id: str) -> Dict[str, Any]:
        ...
        check_files = os.path.exists(pdf_path) and os.path.exists(md_path) and os.path.exists(json_path)
        results["checks"]["files_exist"] = check_files
        if not check_files: results["passed"] = False
        ...
        if os.path.exists(md_path):
            ...
            name_match = client_name in content # Might need normalization but checking raw first
            
            results["checks"]["id_correct"] = id_match
            results["checks"]["name_correct"] = name_match
            if not id_match or not name_match: results["passed"] = False
        ...
        if os.path.exists(arco_dir):
            ...
            if not guia_exists and len(pdfs) > 0: results["passed"] = False
