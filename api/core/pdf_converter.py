...
class PdfConverter:
    ...
    def __init__(self) -> None:
        ...
        self.miktex_user_path = os.path.join(
    ...
    def _update_path_env(self):
        ...
        if platform.system() == "Windows":
            ...
            if os.path.exists(self.miktex_user_path):
                ...
                self.logger.info(
                    f"[*] Temporarily added MiKTeX to PATH: {self.miktex_user_path}"
                )
    ...
    def _install_pandoc(self):
        ...
        print(
            f"[*] Pandoc not found. Attempting automatic installation for {system}..."
        )
    ...
    def _install_miktex(self):
        ...
        print(
            "[*] PDF Engine (LaTeX) appears missing. Attempting MiKTeX Basic installation (Windows Unattended)..."
        )
        try:
            ...
            print(
                "[*] Running MiKTeX silent install (Current User, Basic Package Set)..."
            )
            ...
            if os.path.exists(initexmf_path):
                print(
                    "[*] Configuring MiKTeX to install missing packages on-the-fly..."
                )
                subprocess.run(
                    [initexmf_path, "--set-config-value", "[MPM]AutoInstall=1"],
                    check=False,
                )
    ...
    def convert_to_pdf(self, md_path):
        ...
        if not self._check_pandoc():
            ...
            if not self._check_pandoc():
                return (
                    False,
                    None,
                    "Pandoc installed but not in PATH components (restart required)",
                )
        ...
        try:
            ...
            template_issue = (
                "template" in err_output
                or "LaTeX Error" in err_output
                or "Undefined control sequence" in err_output
            )
            ...
            if missing_latex and platform.system() == "Windows":
                ...
                if self._install_miktex():
                    ...
                    try:
                        print(
                            f"[*] Converting {md_path} to PDF (Attempt 2 - Post MiKTeX Install)..."
                        )
                        run_conversion(use_template=True)
                        print(
                            f"[+] PDF successfully generated (after install): {pdf_path}"
                        )
                        return True, pdf_path, None
                        ...
                        return (
                            False,
                            None,
                            f"Conversion failed after MiKTeX install: {err2}",
                        )
                else:
                    return (
                        False,
                        None,
                        "LaTeX missing and MiKTeX install failed/skipped",
                    )
            ...
            if (template_issue or e.returncode == 43) and os.path.exists(
                fallback_template_path
            ):
                print(
                    "[!] Detailed branding failed. Retrying with SAFE FALLBACK template..."
                )
                try:
                    ...
                    return (
                        True,
                        pdf_path,
                        "Warning: Main template failed, used fallback.",
                    )
