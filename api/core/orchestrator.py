...
class Orchestrator:
    ...
    def run_spiderfoot_scan(self, target: str, scan_id: str) -> List[Dict[str, Any]]:
        ...
        if not os.path.exists(self.sf_script):
            print(
                f"[!] CRITICAL: SpiderFoot not found at {self.sf_script}. Cannot proceed with scan."
            )
            return []
    ...
    def orchestrate(
        self, client_id: str, analysis_type: str = "monthly"
    ) -> Tuple[str, str]:
        ...
        intake_id = self.sm.create_intake(
            client_id, intake_type, requested_by="CLI_USER"
        )
    ...
    def _persist_raw_data(
        self, client_dir: str, intake_id: str, data: List[Dict[str, Any]]
    ) -> str:
    ...
    def _dispatch_notification(
        ...
        if admin_copy not in recipients:
            recipients.append(admin_copy)

        success, msg_id = self.notifier.send_report(
            recipients, pdf_path, client["client_name_full"], scan_id=report_id
        )
        if success:
    ...
    def _handle_qc_failure(self, client_id: str, report_id: str) -> None:
        ...
        self.sm.update_report_status(
            report_id, "INVALIDADO", actor="SYSTEM", invalidated_reason="QC_FAIL"
        )
        rescue_id = self.sm.create_intake(
            client_id, "RESCUE", requested_by="SYSTEM", replaces_report_id=report_id
        )
