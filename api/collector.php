...
foreach ($rssFeeds as $source => $feedUrl) {
    ...
    foreach ($rss->channel->item as $item) {
        if ($itemsProcessed >= 3)
            break; // Limit to top 3 to prevent API exhaustion
        ...
        $stmt = $pdo->prepare("INSERT INTO tactical_feed (title, gemini_summary, severity, source, url, status) VALUES (?, ?, ?, ?, ?, 'UNREAD')");
        $stmt->execute([
            $title,
            $analysis['summary'],
            $analysis['severity'],
